# %%writefile '/content/drive/MyDrive/Colab Notebooks/EQ/EQ-Dataflow_parquet_with_3_pipelines.py'
import os
from google.cloud import storage
import logging
from datetime import datetime
import json
from apache_beam.options.pipeline_options import PipelineOptions, GoogleCloudOptions, StandardOptions, SetupOptions, SparkRunnerOptions
import requests
...
SERVICE_ACCOUNT_KEY = r"C:\Users\Ankush\PycharmProjects\EartQuakeProject\bwt-earthquake-project-a77ea1fdb8a6.json"
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = SERVICE_ACCOUNT_KEY
# Set up project details
current_date = datetime.now().strftime('%Y%m%d')
PROJECT_ID = 'bwt-earthquake-project'
GCS_BUCKET = "eartquake_analysis1"
DATASET_ID = "Earthquake_dataflow"
TABLE_ID = "EarthQuake_table_using_dataflow"
TABLE_REF = f'{PROJECT_ID}.{DATASET_ID}.{TABLE_ID}'

# URLs for API sources
...
# GCS paths
DATAFLOW_LANDING_LOCATION = f"dataflow/historical_raw_data/{current_date}/earthquake_raw_data.json"
DATAFLOW_STAGING_LOCATION = "gs://eartquake_analysis1/dataflow/staging"
DATAFLOW_SILVER_LAYER_PATH = f'dataflow/Historical_Silver_Data/silver/{current_date}/earthquake_silver'
TEMP_LOCATION = "gs://eartquake_analysis1/temp"

# Set up pipeline options
options = PipelineOptions(save_main_session=True, temp_location=TEMP_LOCATION)
google_cloud_options = options.view_as(GoogleCloudOptions)
google_cloud_options.project = PROJECT_ID
google_cloud_options.job_name = f"dataflow-earthquake-{current_date}"
google_cloud_options.region = "asia-east1"
google_cloud_options.staging_location = DATAFLOW_STAGING_LOCATION
google_cloud_options.temp_location = TEMP_LOCATION
options.view_as(StandardOptions).runner = 'DataflowRunner'

logging.info("Successfully configured all settings for the pipeline.")
...
class FetchAndUpload(beam.DoFn):
    ...
    def fetch_data_from_api(self):
        try:
            logging.info(f">> Fetching raw data from API: {self.api_url}")
            response = requests.get(self.api_url)
            response.raise_for_status()
            logging.info(">> SUCCESSFUL: Raw data fetched successfully.")
    ...
    def upload_data_to_gcs(self, data):
        try:
            storage_client = storage.Client()
            bucket = storage_client.bucket(self.bucket_name)
            blob = bucket.blob(self.destination_file_name)
            blob.upload_from_string(json.dumps(data), content_type='application/json')
            logging.info(f">> SUCCESSFUL: Raw data uploaded to GCS at {self.bucket_name}/{self.destination_file_name}")
        except Exception as err:
            logging.error(f">> Failed to upload data to GCS: {err}")
            raise

    def process(self, element):
        data = self.fetch_data_from_api()
        self.upload_data_to_gcs(data)
        output_filename = f"gs://{self.bucket_name}/{self.destination_file_name}"
        ...
        try:
            ...
            for feature in features:
                ...
                result_for_silver_layer = {
                    ...
                    'event_time': datetime.utcfromtimestamp(safe_cast(event_time, int, 0) / 1000).isoformat() if event_time else None,
                    'last_update': datetime.utcfromtimestamp(safe_cast(last_update, int, 0) / 1000).isoformat() if last_update else None,
...
def start_pipeline():
    with beam.Pipeline(options=options) as p:
        ...
        _ = (
            ...
            | 'Fetch and Upload Data' >> beam.ParDo(FetchAndUpload(SOURCE_API_URL_HISTORICAL, GCS_BUCKET, DATAFLOW_LANDING_LOCATION))
        )
    ...
    # Read back the Parquet file from GCS Silver Layer
      read_parquet = (
          ...
          | 'Read From Parquet' >> ReadFromParquet(
              f'gs://{GCS_BUCKET}/{DATAFLOW_SILVER_LAYER_PATH}-00000-of-00001.parquet') # gs://production-bucket-dipakraj/dataflow_silver/20241019/TESTING1-00000-of-00001.parquet
      )
